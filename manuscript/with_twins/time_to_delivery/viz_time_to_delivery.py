...
for index, label_dict in enumerate(up_to_dataset_dict.items()): 
    ...
    _ = plt.plot(interp_fpr, interp_tpr, lw=2, alpha=1, linestyle='-', color=hex_codes[index], label="{} ({:.2f})".format(label.replace("_"," "), auc))
...
for index, label_dict in enumerate(up_to_dataset_dict.items()): 
    ...
    _ = plt.plot(interp_rc, interp_pr, lw=1, alpha=1, linestyle='-', color=hex_codes[index],
             label="{} (AUC={:.2f})".format(label.replace("_"," "), pr_auc))
    
_ = plt.plot([0, 1], [pos_prop, pos_prop], linestyle='-', lw=1,color='#CD5C5C', label='Chance({:.2f})'.format(pos_prop), alpha=.8) 
