...
EGA_FILE="/dors/capra_lab/users/abraha1/projects/PTB_phenotyping/data/combined_cohorts_07_12_2018/full_dataset_characterization/expanded_ega/calc_ega_at_delivery_ega_w_in_20wks_of_delivery.tsv"
...
# output file names
OUTPUT_DIR="/dors/capra_lab/users/abraha1/projects/PTB_phenotyping/data/ptb_predict_machine_learning/feature_matrices/no_twins/time_since_preg_start_icd_cpt_no_twins_timeseries_calc_ega_win_20wk"
...
final_labels_df = ega_df.loc[:, ['GRID','consensus_delivery_date','consensus_delivery_label']].copy()
final_labels_df.columns = ['GRID','delivery_date','label']


delivery_date_dict = dict(zip(ega_df.GRID, ega_df.consensus_delivery_date))
preg_start_date_dict = dict(zip(ega_df.GRID, ega_df.pregnancy_start))
...
timeframes = {'0_weeks':0, '13_weeks':13*7, '28_weeks':28*7, '35_weeks':35*7, '37_weeks':37*7 }
...
for label, feat_df in keep_feat_mat.items():
    ...
    print("counting...")
    grid_df = annotated_df.loc[annotated_df['partition']=='held_out', ['GRID','label']].copy()
    grid_df['twin_bool'] = False
    grid_df.loc[grid_df.GRID.isin(twins_grids_based_on_icd_cpt), 'twin_bool'] = True
    grid_df['delivery_label'] = grid_df.apply(lambda x: get_delivery_label(x),axis=1)
    count_df = pd.value_counts(grid_df.delivery_label).reset_index()
    count_df['timepoint'] = label
...
# write
all_counts_df.to_csv(os.path.join(OUTPUT_DIR, 'all_counts_held_out.tsv'), sep="\t", index=True)
